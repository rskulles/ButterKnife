...
`.claude/launch.json` has two configurations: `butterknife` (plain `dotnet run`) and `butterknife-fake-llm`, which starts the app with the seed backends pointed at `http://127.0.0.1:11434` (only matters for an empty database; otherwise add connections on `/connections`). Pair the latter with a small stub server that answers `/api/tags`, `/api/chat` (NDJSON), `/v1/models`, `/v1/chat/completions` (SSE) and, for the Anthropic kind, `/v1/messages` (Anthropic SSE events) on that port; the unit tests in `tests/ButterKnife.Tests/StubHttp.cs` show the exact wire shapes expected. Browser automation note: synthetic "Return" key presses may not reach the textarea's `keydown` listener; dispatch a `KeyboardEvent('keydown', {key: 'Enter'})` from JS instead when scripting the page.
